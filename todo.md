...
- [ ] In `src/lib/assets.js`, fix 7 entries using wrong paths (`/public/assets/...` or missing leading `/`)
- [ ] Affected: all 5 Work case-study images (`IMG_WORK_1`–`IMG_WORK_5`) and 2 testimonial images (`IMG_TEST_SMALL`, `IMG_TEST_MID_A`)
- [ ] Correct form: `/assets/DT.jpeg`, `/assets/DS.png`, etc. (Vite serves `public/` at root)
- [ ] Verify all images load in production build (`npm run build` + preview)

### 2. Add `<h1>` and fix heading hierarchy
- [ ] Add a single `<h1>` in the hero section (`src/App.jsx` ~line 281) — currently plain `<p>` tags
- [ ] Convert section titles from `<p>` to `<h2>`:
  - [ ] "We help you decide" — `src/components/Services.jsx:115`
  - [ ] "Frequently asked questions" — `src/components/FAQ.jsx:91-92`
  - [ ] "Stories from our partners" — `src/App.jsx:396-397`
  - [ ] "Trusted by / 28 partners" — `src/App.jsx:327-328`
  - [ ] Work section title — `src/components/Work.jsx`
  - [ ] Problems section title — `src/components/Problems.jsx`
- [ ] Remove duplicate `<h2>` text in `src/components/DrxloBento.jsx:72` (duplicates line 29)
- [ ] Ensure order: one `h1` → `h2` sections → `h3` subsections
- [ ] Make footer column headings headings (`src/components/Footer.jsx`) instead of `<p>`
...
- [ ] Create `public/robots.txt`:
  ...
  ```
- [ ] Confirm it appears at `https://drxlo.com/robots.txt` after deploy

### 4. Create `sitemap.xml`
- [ ] Create `public/sitemap.xml` with single URL `https://drxlo.com/` (lastmod optional)
- [ ] Confirm it appears at `https://drxlo.com/sitemap.xml` after deploy
